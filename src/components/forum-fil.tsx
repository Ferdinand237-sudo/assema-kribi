...
type MessageCite = { id: string; content: string; profiles: Auteur | null } | null
      ...
      <div className="mb-6 space-y-3">
        {messages.map((m) => (
          <div key={m.id} id={`message-${m.id}`} className="cadre group border border-black/5 bg-white p-3 pt-4 shadow-sm">
            ...
            {m.message_cite && (
              <a
                href={`#message-${m.message_cite.id}`}
              ...
              >
                <span className="font-medium text-primaire">{nomComplet(m.message_cite.profiles)}</span>
                <span className="ml-1 line-clamp-1 opacity-80">{m.message_cite.content}</span>
              </a>
            )}
